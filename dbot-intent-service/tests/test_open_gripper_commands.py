...
@pytest.mark.open_gripper_commands
@pytest.mark.parametrize('phrase', ['open gripper', 'open hand'])
def test_open_gripper(dbot_intent_parser_fixture, phrase):
    intent = dbot_intent_parser_fixture.calc_intent(f'{phrase}'.format(phrase=phrase))
    assert intent.name == 'open_gripper'
