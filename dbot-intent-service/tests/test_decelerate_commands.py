...
@pytest.mark.decelerate_commands
@pytest.mark.parametrize('phrase', ['slower', 'slow down', 'slow down!', 'slower dbot'])
def test_decelerate(dbot_intent_parser_fixture, phrase):
    intent = dbot_intent_parser_fixture.calc_intent(f'{phrase}'.format(phrase=phrase))
    assert intent.name == 'decelerate'
